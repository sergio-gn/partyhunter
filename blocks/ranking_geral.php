<?php
// Query para pegar os posts ordenados pelo campo "vote"
$args = array(
    'post_type' => 'post',
    'posts_per_page' => -1,
    'meta_key' => 'vote',
...
// Get all categories that are assigned to posts (cities)
...
if ($query->have_posts()) {
    while ($query->have_posts()) {
        ...
        foreach ($categories as $category) {
            if (!isset($all_categories[$category->term_id])) {
                $all_categories[$category->term_id] = $category->name;
            }
        }
    }
    // Rewind the query so we can loop through posts again
    $query->rewind_posts();
}
...
<section class="ranking_city">
    <div class="container">
        <?php if (!empty($all_categories)) : ?>
            <div class="city-filter-wrapper" style="color:#fff;margin-bottom: 2rem;">
                <label for="city-filter" style="display: block; margin-bottom: 0.5rem; font-weight: bold;">Filtrar por cidade:</label>
                <select id="city-filter" style="padding: 0.5rem 1rem; font-size: 1rem; border: 1px solid #ccc; border-radius: 4px; min-width: 200px;">
        ...
        <?php if ($query->have_posts()) :
            ...
            while ($query->have_posts()) : $query->the_post();
                ...
                // Assign classes for the first, second, and third places
                $place_class = '';
                if ($position == 1) {
                ...
                // Get post categories (cities) for filtering
        ...
        ?>
                <div class="post-card <?php echo $place_class; ?>" data-categories="<?php echo esc_attr($category_ids_str); ?>">
...
<script>
    ...
    if (cityFilter) {
        cityFilter.addEventListener("change", function() {
            ...
            postCards.forEach((card, index) => {
                ...
                const shouldShow = selectedCityId === "" || cardCategories.includes(selectedCityId);
                
                if (shouldShow) {
                    card.style.display = "";
                    visibleCount++;
                    
                    // Update position classes for visible posts
                    const title = card.querySelector("h2");
                    if (title) {
                        title.classList.remove("first-place", "second-place", "third-place");
                        if (visibleCount === 1) {
                            title.classList.add("first-place");
                        } else if (visibleCount === 2) {
                            title.classList.add("second-place");
                        } else if (visibleCount === 3) {
                            title.classList.add("third-place");
                        }
                    }
                } else {
    ...
    buttons.forEach(button => {
        button.addEventListener("click", function () {
            ...
            // Get the user's previous vote status for this post
